...
exports.chatUpdate = async(conn, m) => {
    ...
    if (m.key) {
      m.isBaileys = m.key.id.startsWith('3EB0') && m.key.id.length === 12
      m.chat = m.key.remoteJid
      m.sender = m.key.fromMe ? conn.user.jid : m.participant ? m.participant : m.key.participant ? m.key.participant : m.chat
    ...
    if (m.message) {
        ...
        if (m.messageType === 'ephemeralMessage') {
            exports.chatUpdate(conn, m.messageContent)
        ...
        if (m.quoted) {
            ...
            m.quoted.chat = m.messageContent.contextInfo.remoteJid || m.chat
      ...
      if (m.messageContent.url) m.download = (type = 'buffer') => conn.downloadM(m, type)

      m.text = (m.messageType == 'listResponseMessage' ? m.messageContent.singleSelectReply.selectedRowId : '') || m.messageContent.text || m.messageContent.caption || m.messageContent || ''
